...
if __name__ == "__main__":
    ...
    for benchmark in benchmarks[args.dataset]:
        for part in parts:
            ...
            try:
                data = np.load('/data/xcl_data/{}/{}_data_joint.npy'.format(benchmark, part), mmap_mode='r')
                N, C, T, V, M = data.shape
                fp_sp = open_memmap(
                     '/data/xcl_data/{}/{}_data_bone.npy'.format(benchmark, part),
                    dtype='float32',
                    mode='w+',
                    shape=(N, 3, T, V, M))
